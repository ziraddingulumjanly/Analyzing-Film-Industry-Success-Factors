...
ggplot(movies_by_year_genre, aes(x = year, y = movie_count, fill = genre)) +
  ...
  theme(
    ...
    legend.title = element_te# Load necessary libraries
library(ggplot2)
library(dplyr)
...
# Create a faceted plot for number of movies released by year and genre
ggplot(movies_by_year_genre, aes(x = year, y = movie_count)) +
  ...
  facet_wrap(~genre, scales = "free_y")  # Facet by genre with free y-axis scaling
